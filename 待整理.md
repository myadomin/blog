...
todo 
-- babel-loader - .bablerc.js - babel-polyfill ，babel-polyfill对比babel-plugin-transform-runtime 参考知乎三个收藏
-- https通讯协议，基于chormeF12 请求头和响应头 比如set-cookie max-age等 坑：max-age的单位是秒
-- vue底层原理 250行mvvm系统 
-- axios拦截器响应做前端捕获，实例：通讯录发版 点击搜索没反应 后来发现是请求后台返回200但是code -1, 所以应该做整体的前端错误捕获 这种情况弹message
-- 尽量不做nginx的路由映射 否者发版生产也要配nginx
这里因为Home组件本身就是路由组件(因为他是<Route path="/" component={Home} />定义的) 所以即使不withRouter，props也有location, history
计算机基础，排序算法、时间复杂度概念等；
，state 更新机制；
两个大数相加；
验证一个域名属于qq.com包括子域名；
